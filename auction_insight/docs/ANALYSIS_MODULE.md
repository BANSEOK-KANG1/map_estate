...
- `app/analysis/rights_eval.py` · `rights_service.py` · `loan_plan.py` · `lot_link.py`
- `tests/test_money_calculator.py` · `tests/test_analysis_models.py` · `tests/test_pdf_classify.py` · `tests/test_rights_eval.py` · `tests/test_loan_plan.py` · `tests/test_lot_link.py`
...
### Phase 5 (공식 데이터 · AuctionLot 링크)
```
POST /api/analysis/items/from-lot/{lot_id}   # 스크리닝 lot → 분석 아이템 (기본 재사용)
GET  /api/analysis/items/{id}               # linked_lot 시세·인프라 스냅샷 포함
```

- 만원→원 변환, 주소·감정/최저가·좌표·위험칩 가져오기.
- 온비드 임대차 목록은 OccupancyClaim **HOLD 스텁**만 (문서 없이 확정 금지).
- 물건 상세 「심층 분석 열기」→ 분석 랩 7탭.

저장: `DocumentStore` — `local` (기본) 또는 `s3`/`r2` (`boto3` + endpoint).  
선택: `ANALYSIS_API_KEY`로 분석 쓰기 API 보호.
...
| **6** | 테스트·보안·배포 — [PHASE6_SECURITY_DEPLOY.md](PHASE6_SECURITY_DEPLOY.md) |
