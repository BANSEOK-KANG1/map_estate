...
| 구성 | 필드 | 기본 가중치 | 역할 |
|------|------|-------------|------|
| 감정 할인 | `discount_vs_appraisal` → 점수화 | **32%** | 최저가 vs 감정가 |
| 시세 할인 | `discount_vs_market` → 점수화 | **28%** | 최저가 vs 인근 실거래 (결측 시 제외) |
| 상권·인프라 | `infra_score` | **18%** | 카카오 POI (미측정 시 제외) |
| 마감 긴급도 | `urgency_score` | **12%** | 마감까지 일수 (half-life 18일) |
| 유찰 | `fail_count` → 점수화 | **10%** | 유찰 횟수 가산 |
| **종합** | `total_score` | 재정규화 | 있는 항목만 가중합 |

> 구버전은 시세 결측=50·인프라 0·마감≥30일=10·감정≥40%=100 → **전부 44점**으로 붕괴. v2에서 수정.
